...
- **Shared truth over local truth.** One definition of throughput and quality that held across every project, tool, customer, and time zone, instead of per-team spreadsheets that disagreed at delivery review.
- **Early warning, not post-mortem.** Quality and throughput problems had to surface while a program could still be corrected, not after a customer found them.
- **Edge cases closed, not just captured.** Every captured case needed a resolution path and a route back into training. Capture volume alone was not the measure.
